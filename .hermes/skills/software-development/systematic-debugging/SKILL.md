...
### 6. Classify Rendering Bugs by Variable Isolation

**WHEN the symptom is visual and seems device-specific (OS, browser, monitor, phone):**

Build the tight loop around the exact matrix the user described instead of collapsing everything into one theory.

- Separate the variables explicitly: **OS**, **browser**, **display/monitor**, **deployment vs local**, **effects on/off**, **font path / asset type**.
- If the same artifact appears on local and public builds, bias toward **application/rendering** causes, not server/deploy causes.
- If the problem follows one OS across multiple displays, bias toward **OS/browser rendering stack** rather than monitor geometry.
- If turning off visual FX (`glitch`, `CRT`, filters, transforms) does not change the symptom, retire the FX hypothesis quickly and move down-stack.
- For Unicode-art banners, terminal art, block/braille glyph compositions, or other decorative text that depends on exact glyph metrics, treat **cross-platform font/glyph rendering fragility** as a first-class root-cause candidate.
- When the artifact is purely decorative/branding and robust cross-platform fidelity matters more than text semantics, prefer a **raster or SVG asset** over continued font-stack tweaking.

**Pitfall:** Do not stop at the first plausible rendering theory. In this session, the first theory (glitch/CRT distortion) looked plausible from one screenshot, but the user's control test (`effects off`, still distorted) was the decisive falsifier. Bake the user's control test into the loop and update the ranking immediately.
